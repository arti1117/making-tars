...
def conv2d_simple(x, W, b=None, stride=1, pad=0):
    ...
    SH, SW = pair(stride)
    PH, PW = pair(pad)
    OH = dezero.utils.get_conv_outsize(H, KH, SH, PH)
    OW = dezero.utils.get_conv_outsize(W, KW, SW, PW)
...
class Deconv2d(Function):
    ...
    def forward(self, x, W, b):
        ...
        if self.outsize is None:
            out_h = dezero.utils.get_deconv_outsize(H, KH, SH, PH)
            out_w = dezero.utils.get_deconv_outsize(W, KW, SW, PW)
        else:
...
def _im2col_gpu(img, kernel_size, stride, pad):
    ...
    sy, sx = pair(stride)
    ph, pw = pair(pad)
    out_h = dezero.utils.get_conv_outsize(h, kh, sy, ph)
    out_w = dezero.utils.get_conv_outsize(w, kw, sx, pw)
    dy, dx = 1, 1
